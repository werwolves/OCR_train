...
class My_warpPerspective:
    def __init__(self, padding_size=(0, 0, 0, 0)):
    ...
    def do(self, cv_img, label_contents=None):
        '''
        ...
        random_border_x = random.randint(1, 3)
        random_border_y = random.randint(1, 3)
        ######################################################
        # 1  平行四边形 1
        ...
        #         trans_point = self.point_map(M=M, point=point_list)
        #         new_box += str(min(max(0, trans_point[0]), transform_img_w - 2))  # 0 < x < 变换后图像的宽
        #         new_box += ','
        #         new_box += str(min(max(0, trans_point[1]), transform_img_h - 2))  # 0 < y < 变换后图像的高
        #         new_box += ','
        #     new_box += cls
        #     new_trans_label_contents.append(new_box)
        #
        # return transform_img, new_trans_label_contents
    ...
    for i in os.listdir(img_path):
        ...
        for j in deal_label:
            ...
            cv2.line(deal_img, (int(float(box_info[0])), int(float(box_info[1]))),
                     (int(float(box_info[2])), int(float(box_info[3]))), (0, 0, 255), 1)
