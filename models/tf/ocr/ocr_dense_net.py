...
class TFOcrDenseNetModelDef(TFCtcOcrModelDef):
    def __init__(self, name, **kwargs):
        super().__init__(name, **kwargs)
        # self.dense_block_layers = self.args.get('dense_block_layers', [8, 8, 8])   # 原版的
        self.dense_block_layers = self.args.get('dense_block_layers', [8, 8, 8])
        self.dense_block_growth_rate = self.args.get('dense_block_growth_rate', 8)
        # self.first_conv_filters = self.args.get('first_conv_filters', 64)
        self.first_conv_filters = self.args.get('first_conv_filters', 64)

        self.first_conv_size = self.args.get('first_conv_size', 5)
        self.first_conv_stride = self.args.get('first_conv_stride', 2)
        #self.first_conv_stride = self.args.get('first_conv_stride', 1) # hou_2021_7_28

        self.first_pool_size = self.args.get('first_pool_size', 0)
        self.first_pool_stride = self.args.get('first_pool_stride', 2)

        self.last_conv_size = self.args.get('last_conv_size', 0)
        self.last_conv_filters = self.args.get('last_conv_filters', 0)
        self.last_pool_size = self.args.get('last_pool_size', 2)

        self.trans_block_filters = self.args.get('trans_block_filters', 128)
    ...
    def transition_block(cls, input, nb_filter, dropout_rate=None, pooltype=1, weight_decay=1e-4, is_test=False):
        ...
        if (pooltype == 2):
            ...
            x = tf.keras.layers.AveragePooling2D((2, 2), strides=(2, 2))(x) # 按照类似论文的方式尝试  
        elif (pooltype == 1):
    ...
    def dense_net_to_seq(self, input, is_test=False):
        ...
        _dropout_rate = 0.2
        _weight_decay = 1e-4
        print('防止过拟合要调的参数。。。。。。。。。。is_test: %s。。。。。。。。。。。'%(is_test))
        ...
        for n_layer in nb_layers[:-1]:      # 跳出for循环的时--- shape=(4, 3, 28, 128)
            ...
            x, nb_filter = self.transition_block(x, trans_block_filters, _dropout_rate, 2, _weight_decay,
                                                 is_test=is_test)
...
class TFOcrDenseNetLstmModelDef(TFOcrDenseNetModelDef):
    ...
    def build_model_outputs(self, input, **kwargs):
        ...
        x = self.dense_net_to_seq(input)  # (batch,width/8,x)
