...
class TFOcrModelDef(TFModelDef, OcrModelDef):
    ...
    def build_model(self, input=None, **kwargs):
        ...
        self.outputs = tf.keras.layers.Dense(nclass, name='out', activation='softmax')(x)
        base_model = tf.keras.Model(inputs=input, outputs=self.outputs)
        base_model.summary()
...
class TFCtcOcrModelDef(TFOcrModelDef):
    ...
    def build_model(self, **kwargs):
        ...
        if kwargs.get('pretraining', True):
            weights_path = 'E:/PycharmProjects/work2/ai-training/pretraining_model/full.7.h5'
        ...
        #打印 模型中layer的trainable状态
        layer_trainable_list=[]
        for layer in base_model.layers:
            if layer.trainable:
                layer_trainable_list.append(1)
            else:
                layer_trainable_list.append(0)

        print("模型中layer的trainable状态:")
        print(layer_trainable_list)
...
class TFCtcTrainModel(TFTrainModel):
    ...
    def init_model(self):
        self.base_model = self.model_def.build_model()
        ...
        trainable_var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'dense')
        self.train_model.compile(loss={'ctc': lambda y_true, y_pred: y_pred}, optimizer='adam', metrics=['accuracy'])
...
class TFCtcInferModel(TFInferModel):
    ...
    def get_out_names(self):
        ...
        # return ['out_idxes/SparseToDense:0','out_idxes/SparseToDense/default_value:0']
    ...
    def build_infer_model(self):
        ...
        # base_model_no_ctc = tf.keras.Model(inputs=[self.model_def.input], outputs=y_pred)
        ...
        # noctc_pb_file = FileUtils.change_ext_name(h5_file, '_no_ctc.pb')
        # self.model_def.convert_model_to_pb(base_model_no_ctc, noctc_pb_file)
