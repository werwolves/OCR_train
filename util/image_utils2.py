...
class ImageTools:
    ...
    def resize_and_pad(im, h, w):
        ...
        '''
        im_h, im_w = im.shape[:2]
        if im_h == h and im_w == w:     # 图片和输入参数完美的一致
            return (im, 1.0)

        sy = 1.0 * h / im_h
        sx = 1.0 * w / im_w
        s = min(sx, sy)
        # 可见图片的新的宽度和高度 都是 小于或等于设置的 h，w 的 然后通过
        new_h = int(s * im_h)
        new_w = int(s * im_w)
    ...
    @staticmethod
    def resize_and_pad_img(im, h, fill_width, pad_left_per=0.5):
        '''
        根据高度缩放图像，宽度右边补充白色背景
        :param self:
        :param im: 数像：nparray
...
class ImageRndUtils:
    ...
    @staticmethod
    def updown_pad(img, margin, fill_color=(255, 255)):
        im = Image.fromarray(img)
        w, h = im.size
        l = margin[0]
        t = margin[1]
    ...
    def updown_pad2(img, margin, fill_color=(255, 255)):
        im = Image.fromarray(img)
        w, h = im.size
        l = margin[0]
        t = margin[1]
